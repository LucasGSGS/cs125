//UIUC CS125 FALL 2013 MP. File: CheckInputOutput.java, CS125 Project: Challenge7-RecursiveKnight, Version: 2013-11-12T09:02:55-0600.414969527
 ...
 * Updated 9/11/2008 : Auto test discovery.
 * Updated 9/9/2008 : Changed output format.
 * @author angrave
 */
public class CheckInputOutput {
	...
	/**
	 * Returns true if the given .java file contains a valid @author javadoc tag
	 * @param file , if the @author tag includes this value, it is presumed to be invalid
	 * @param ignoreDefault
	 * @return
			...
			while ((line = reader.readLine()) != null) {
				...
				if (line.contains(ignoreDefault)) {
					System.err
							.println("File "
									+ file
									+ " : @author comments at the beginning should include your netid");
					return false;
				}
				// else implied
				if( line.contains("@author") && !line.trim().endsWith("@author") ) return true;
			}
		} catch (Exception e) {
	...
	static String getTestName() {
		...
		if(trace == null) return "test?";
		for(StackTraceElement e : trace) {
			String method = e.getMethodName();
			if( method!=null && method.startsWith("test") && method.length() >4)
